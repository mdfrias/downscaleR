...
install_github("SantanderMetGroup/downscaleR@stable")
...
```
```{r,eval=FALSE,echo=FALSE}
save(prd, file = "tutorials/prd.Rdata")
```
```{r,eval=FALSE}
...
```
```{r,echo=FALSE}
load("/home/dfrias/github/downscaleR_mdfrias/tutorials/prd.Rdata")
```
...
## 2: In interpGridData(obs, new.grid = getGrid(prd), method = "nearest") :
##   The new latitudes are outside the data extent
```
```{r,echo=FALSE,eval=FALSE}
save(obs, file = "tutorials/obs.Rdata")
```
```{r,echo=FALSE}
load("/home/dfrias/github/downscaleR_mdfrias/tutorials/obs.Rdata")
```
